            ...
            <div class="w-100">
                <div class="mb-3">
                    <x-form-label id="name{{ $employee['id'] }}"
                        label="Nama Karyawan (ID: {{ $employee['id'] }})" />
                ...
                <div class="mb-3">
                    <x-form-label id="email{{ $employee['id'] }}" label='Email Karyawan' />
                ...
                <div class="mb-3">
                    ...
                    <x-form-input id="password{{ $employee['id'] }}" name="password{{ $employee['id'] }}"  placeholder="Masukkan Password"
                        wire:model.defer="employees.{{ $loop->index }}.password" type="password" />
                    <x-form-error key="employees.{{ $loop->index }}.password"  />
                </div>

                @if (auth()->user()->role_id == 1 || auth()->user()->role_id == 2 )
                <div class="mb-3">
                    <x-form-label id="position_id{{ $employee['id'] }}"
                        label='Jabatan / Posisi Karyawan {{ $loop->iteration }}' />
